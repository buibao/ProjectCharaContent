...
$aColumns = [
    ...
    // FIX SOURCE FILED DONE
     '(SELECT GROUP_CONCAT(name ORDER BY name ASC) FROM tblcustomerfields JOIN tblcustomerfields_in ON tblcustomerfields_in.fieldid = tblcustomerfields.id WHERE customer_id = tblclients.userid LIMIT 1) as fields',
...
// }
// if (count($fieldIds) > 0) {
//     array_push($filter, 'AND tblclients.userid IN (SELECT customer_id FROM tblcustomerfields_in WHERE fieldid IN (' . implode(', ', $fieldIds) . '))');
// }
...
foreach ($rResult as $aRow) {
    ...
    // Customer fields parsing
        $fieldRow = '';
    if ($aRow['fields']) {
        $fields = explode(',', $aRow['fields']);
        foreach ($fields as $field) {
            $fieldRow .= '<span class="label label-default mleft5 inline-block customer-group-list pointer">' . $field . '</span>';
        }
    }
