	...
	public function view($id)
	{
		...
		if ($post_id) {
			...
			for ($i = 0; $i < 6; $i++) {
				...
				$result = $this->curl_api->graph_api($url,$method,$data);
		...
		$url1 = "https://graph.facebook.com/" . $post_id . '?access_token=' . $user_access_token;
		$method1 = "GET";
		$data1 = array();

		$result = $this->curl_api->graph_api($url1,$method1,$data1);
		$checkAccessToken = json_decode($result, true);
	...
	public function post_content()
	{
		...
		if (!empty($_POST['id_page'])) {
			...
			if ($checkAccessToken['access_token']) {
				...
				$result1 = $this->curl_api->graph_api($url1,$method1,$data);
				...
				if ($checkImage['post_id']) {
					...
					// Get task ID
					$taskID = $this->contents_model->getTaskId($id)->task_title;
					// Get count total from tblstafftasks
					$countTotal = $this->contents_model->getCount($taskID)->count;
					// Get total content complete with status = 5 and task_title = $taskID
					$countComplete = $this->contents_model->get_Count_Content_By_TaskID($taskID);
