...
//Filter Project Type
$types    = $this->ci->projects_model->get_project_types();
$typesIds = [];
foreach ($types as $type) {
    if ($this->ci->input->post('projects_by_type_' . $type['id'])) {
        array_push($typesIds, $type['id']);
    }
}
if (count($typesIds) > 0) {
    array_push($filter, 'AND project_type IN (' . implode(', ', $typesIds) . ')');
}
...
foreach ($rResult as $aRow) {
    ...
    $staff_ids = $this->ci->projects_model->get_project_members($aRow['id']); // Only Display User's Project 
    foreach($staff_ids as $staff_id)
    {
      $user2 = $staff_id['staff_id']; 
      $membersOutput = '';
     if($user_id == $user2)
    {
    $row = [];
