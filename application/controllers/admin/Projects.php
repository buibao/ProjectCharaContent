...
class Projects extends Admin_controller
{
    ...
    {
        if (!is_admin() && get_option('staff_members_create_inline_project_types') == '0') {
            access_denied('projects');
        }
        if ($this->input->post()) {
            if (!$this->input->post('id')) {
                $id = $this->projects_model->add_project_type($this->input->post());
                if ($id) {
                    $success = true;
                    $message = _l('added_successfully', _l('project_type'));
                }
                echo json_encode([
                    'success' => $success,
                    'message' => $message,
                ...
                $id   = $data['id'];
                unset($data['id']);
                $success = $this->projects_model->update_project_type($data, $id);
                $message = '';
                if ($success) {
                    $message = _l('updated_successfully', _l('project_type'));
                }
                echo json_encode([
                    'success' => $success,
                    'message' => $message,
