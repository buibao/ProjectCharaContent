...
<div class="modal fade" id="customer_field_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button group="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">
                    <span class="edit-title"><?php echo _l('customer_field_edit_heading'); ?></span> 
                    <span class="add-title"><?php echo _l('customer_field_add_heading'); ?></span>
                </h4>
            </div>
            <?php echo form_open('admin/clients/field',array('id'=>'customer-field-modal')); ?>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        <?php echo render_input('name','customer_field_name'); ?>
                        <?php echo form_hidden('id'); ?>
                    </div>
                </div>
            </div>
...
<script>
    window.addEventListener('load',function(){
       _validate_form($('#customer-field-modal'), {
        name: 'required'
    }, manage_customer_fields);

       $('#customer_field_modal').on('show.bs.modal', function(e) {
        var invoker = $(e.relatedTarget);
        var fields_id = $(invoker).data('id');
        $('#customer_field_modal .add-title').removeClass('hide');
        $('#customer_field_modal .edit-title').addClass('hide');
        $('#customer_field_modal input[name="id"]').val('');
        $('#customer_field_modal input[name="name"]').val('');
        // is from the edit button
        if (typeof(fields_id) !== 'undefined') {
            $('#customer_field_modal input[name="id"]').val(fields_id);
            $('#customer_field_modal .add-title').addClass('hide');
            $('#customer_field_modal .edit-title').removeClass('hide');
            $('#customer_field_modal input[name="name"]').val($(invoker).parents('tr').find('td').eq(0).text());
        }
    });
   });
    function manage_customer_fields(form) {
        var data = $(form).serialize();
        var url = form.action;
        $.post(url, data).done(function(response) {
            response = JSON.parse(response);
            if (response.success == true) {
                if($.fn.DataTable.isDataTable('.table-customer-fields')){
                    $('.table-customer-fields').DataTable().ajax.reload();
                }
                if($('body').hasClass('dynamic-create-groups') && typeof(response.id) != 'undefined') {
                    var fields = $('select[name="lv_fields_in[]"]');
                    fields.prepend('<option value="'+response.id+'">'+response.name+'</option>');
                    fields.selectpicker('refresh');
                }
                alert_float('success', response.message);
            }
            $('#customer_field_modal').modal('hide');
        });
        return false;
    }
