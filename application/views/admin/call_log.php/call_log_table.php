...
$aColumns = [
    ...
    'datestart',
    'dateend',
    'tblcontracts.contract_status as status',
    ];
...
$statuses  = $this->ci->contracts_model->get_statuses();
$statusIds = [];

foreach ($statuses as $status) {
    if ($this->ci->input->post('contract_status_' . $status['id'])) {
...
foreach ($rResult as $aRow) {
    ...
    $row[] = _d($aRow['dateend']);
     ...
     $row[] = format_contract_status($aRow['status']);
