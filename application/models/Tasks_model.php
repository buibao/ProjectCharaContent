...
class Tasks_model extends CRM_Model
{
	...
	//update getcontent
            $task->contents        = $this->get_contents($id);
            //end fix
        ...
        return do_action('get_task', $task);
	     ...
	     //Update get_contents
    public function get_contents($taskid)
    {
        $this->db->where('task_title', $taskid)->where("(status='4' OR status='5')");
        
        return $this->db->get('tblcontents')->result_array();
    }
    //end fix
    ...
    public function update($data, $id, $clientRequest = false)
    {
  	...
  	$datas['datestart'] =  $data['startdate'];
        $datas['dateend'] =    $data['duedate'];
        $datas['task_id'] =    $data['approveId'];

        $this->db->where('task_title',$id);
        $this->db->update('tblcontents', $datas);
