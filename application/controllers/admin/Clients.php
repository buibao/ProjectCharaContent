...
class Clients extends Admin_controller
{
    ...
    public function client($id = '')
    {
        ...
        if ($id == '') {
            ...
            if ($group == 'profile') {
                ...
                // FIX SOURCE FIELD
                 $data['customer_fields'] = $this->clients_model->get_customer_fields($id);
                            ...
                            if (count($insert) > 0) {
                                ...
                                if (!$this->input->post('simulate')) {
                                    ...
                                    if ($clientid) {
                                        ...
                                        // FIX SOURCE
                                        if ($this->input->post('lv_fields_in[]')) {
                                            $lv_fields_in = $this->input->post('lv_fields_in[]');
                                            foreach ($lv_fields_in as $field) {
    ...
    public function groups()
    {
        ...
        if ($this->input->is_ajax_request()) {
         ...
         //   $this->app->get_table_data('customers_fields');
    ...
    {
        if (!is_admin()) {
            access_denied('Customer Fields');
        }
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('customers_fields');
        }
        $data['title'] = ('customers_fields');
        $this->load->view('admin/clients/fields_manage', $data);
    }

     // FIX SOURCE FIELD DONE + TASK #10
    public function field()
    {
        if (!is_admin() && get_option('staff_members_create_inline_customer_fields') == '0') {
            access_denied('Customer Fileds');
        }

        if ($this->input->is_ajax_request()) {
            $data = $this->input->post();
            if ($data['id'] == '') {
                $id      = $this->clients_model->add_field($data);
                $message = $id ? _l('added_successfully', _l('customer_field')) : '';
                echo json_encode([
                    'success' => $id ? true : false,
                    'message' => $message,
                    'id'      => $id,
                ...
                $success = $this->clients_model->edit_field($data);
                $message = '';
                if ($success == true) {
                    $message = _l('updated_successfully', _l('customer_field'));
                }
                echo json_encode([
                    'success' => $success,
                    'message' => $message,
    ...
    {
        if (!is_admin()) {
            access_denied('Delete Customer Fields');
        }
        if (!$id) {
            redirect(admin_url('clients/fields'));
        }
        $response = $this->clients_model->delete_field($id);
        if ($response == true) {
            set_alert('success', _l('deleted', _l('customer_field')));
        } else {
            set_alert('warning', _l('problem_deleting', _l('customer_field_lowercase')));
        }
        redirect(admin_url('clients/fields'));
    }
