...
class Post_contents extends Admin_controller
{
	...
	public function view($id)
	{
		...
		$url = "https://graph.facebook.com/746769405744635_783668245388084?access_token=" . $user_access_token;
		$ch = curl_init();
		$method = "GET";
		$data1 = array();
		curl_setopt($ch, CURLOPT_URL, $url);
	...
	public function post_content()
	{
		$success = false;
		$message = '';
		if (!empty($_POST['id_page']) && !empty($_POST['id'])) {
			$id = $_POST['id'];
			...
			if ($checkAccessToken['access_token']) {
				...
				if ($checkImage['post_id']) {
					...
					if ($success) {
						set_alert('success', $caption, _l('content'));
						echo json_encode([
							'success' => $success,
							'message' => $message,
						]);
